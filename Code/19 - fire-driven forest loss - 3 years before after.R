...
for(i in 1:nrow(sf_fires)){
  ...
  # get rasters 3 years before and after fire, then stack
  ras_yearBefore1 <- rast_woody[paste0('wood_', year_i - 1)]
  ras_yearBefore2 <- rast_woody[paste0('wood_', year_i - 2)]
  ras_yearBefore3 <- rast_woody[paste0('wood_', year_i - 3)]
  ras_yearAfter1  <- rast_woody[paste0('wood_', year_i + 1)]
  ras_yearAfter2  <- rast_woody[paste0('wood_', year_i + 2)]
  ras_yearAfter3  <- rast_woody[paste0('wood_', year_i + 3)]
  ras_yearBefore <- (ras_yearBefore1 + ras_yearBefore2 + ras_yearBefore3) / 3
  ras_yearAfter  <- (ras_yearAfter1 + ras_yearAfter2 + ras_yearAfter3) / 3
  gc()
...
saveRDS(df_forestLoss,
        file = paste0('H:/My Drive/Projects/PICASC Land-to-sea/Data/Intermediate/Fire/',
                      '19 - fire-driven forest loss and fire risk - 3 years before after.rds'))
df_forestLoss <-
