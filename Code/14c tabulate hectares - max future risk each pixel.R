    ...
    for(m in 1:length(vec_models)){
      for(r in 1:length(vec_rcps)){
        for(y in 1:length(vec_years)){
          ...
          dat <- dplyr::left_join(dat_maxRisk, fireRiskLevels, 'maxFireRisk')
