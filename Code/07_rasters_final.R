...
##### current fire prob - max #####

# list current fire prob rasters without SE rasters
raster_filenames <- list.files('H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/prediction_rasters_mean', pattern = 'monthly')
...
for(i in 1:length(raster_groups)){
  ...
  # get wet and dry season maxes
  rasters_wet <- stack(rasters_wet); rasters_wet_mean <- calc(rasters_wet, max)
  rasters_dry <- stack(rasters_dry); rasters_dry_mean <- calc(rasters_dry, max)
...
### for all rasters, remove risk pixels if landcover is NA ###

# load a landcover raster
raster_landcover <- raster("H:/My Drive/Projects/PICASC Land-to-sea/Data/Intermediate/Fire/00 Statewide landcover rasters resampled/HI_FracLC_bare_2016.tif")

# if landcover is NA, change risk to NA
for(i in 1:length(raster_list_dry)){
  
  # crop raster
  raster_landcover_cropped <- crop(raster_landcover, raster_list_dry[[i]])
  
  # change risk to NA if landcover is NA
  raster_list_dry[[i]][is.na(raster_landcover_cropped) & !is.na(raster_list_dry[[i]])] <- NA
  raster_list_wet[[i]][is.na(raster_landcover_cropped) & !is.na(raster_list_wet[[i]])] <- NA
  
}

rm(raster_landcover_cropped); gc()
...
# export island-level rasters
writeRaster(raster_statewide_dry, filename = 'H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/rasters_final/max_current_fire_prob_dry_statewide.tif', overwrite = TRUE)
writeRaster(raster_statewide_wet, filename = 'H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/rasters_final/max_current_fire_prob_wet_statewide.tif', overwrite = TRUE)

for(i in 1:length(raster_list_dry)){
  
  writeRaster(raster_list_dry[[i]], filename = paste0('H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/rasters_final/max_current_fire_prob_dry_', raster_groups[[i]], '.tif'), overwrite = TRUE)
  writeRaster(raster_list_wet[[i]], filename = paste0('H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/rasters_final/max_current_fire_prob_wet_', raster_groups[[i]], '.tif'), overwrite = TRUE)
...
# get raster names
list_seasonalRasters <-
  list.files('H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/rasters_final/',
             pattern = 'max_current', full.names = TRUE)

# for each island, save max annual
for(i in 1:length(island)){
  
  # get rasters for island i
  list_ras_names <- list_seasonalRasters[grep(island[[i]], list_seasonalRasters)]
  list_ras <- lapply(list_ras_names, raster)
  
  # get max annual risk
  ras_maxAnnual <- calc(stack(list_ras), max)
  
  # write raster
  writeRaster(ras_maxAnnual,
              filename = paste0('H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/rasters_final/max_current_fire_prob_annual_',
                                island[[i]], '.tif'),
              overwrite = TRUE)
}

rm(list_seasonalRasters, i, list_ras, list_ras_names, ras_maxAnnual)
gc()

# combine into statewide raster and export
list_ras_names <-
  list.files('H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/rasters_final/',
             pattern = 'max_current_fire_prob_annual_',
             full.names = TRUE)
list_ras <- lapply(list_ras_names, raster)
raster_statewide_annual <- do.call(raster::merge, list_ras)
writeRaster(raster_statewide_annual,
            filename = 'H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/rasters_final/max_current_fire_prob_annual_statewide.tif',
            overwrite = TRUE)
