...
# NEW MAUI MODELS
load("H:/My Drive/Projects/PICASC Land-to-sea/Data/Intermediate/Fire/2021_09_FIRE_Hawaii_all_isl_EXTRACTION/Maui model comparison/maui_models.Rdata")

# create list of models
model_list <- list(m1.3.1.mn, m1.3.2.mn, m1.3.3.mn, m1.3.4.mn, m1.3.5.mn)
names(model_list) <- c('m1.3.1.mn', 'm1.3.2.mn', 'm1.3.3.mn', 'm1.3.4.mn', 'm1.3.5.mn')
...
for(i in 4:4){
  for(m in 13:length(month)){  # 4:length(month)
    for(M in 1:length(model_list)){  # 1:length(model_list)
      
      # herb cover, wood cover, soil moisture, max temp, annual temp, and ignition rasters
      barecov_yearof <- raster(paste0('H:/My Drive/Projects/PICASC Land-to-sea/Data/Intermediate/Fire/raster_stacks/barecov_yearof/barecov_yearof_', island[[i]], '_2016_.tif'))
      names(barecov_yearof) <- 'barecov_yearof'
      ...
      # create stack
      raster_stack <- stack(list(barecov_yearof, herbcov_yearof, woodcov_yearof, HI_EVAP_min_monthly_soil_mst, maxTemp_C, mean_annual_temp,
      ...
      pred.vals <- predict(model_list[[M]], newdata = dat.df, progress = 'text', type = 'response', se.fit = TRUE)
      ...
      writeRaster(raster_predVals, filename = paste0('H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/prediction_rasters_mean_MauiBare/',
                                                     'monthly_mean_fire_prob_', island[[i]], '_', names(model_list)[[M]], '_month', month[[m]], '.tif'), overwrite = TRUE)
      
      raster_predSEs <- herbcov_yearof  # choose random raster to place pred vals into
      values(raster_predSEs) <- c(pred.vals$se.fit)
      writeRaster(raster_predSEs, filename = paste0('H:/My Drive/Projects/PICASC Land-to-sea/Data/Processed/Fire/prediction_rasters_mean_MauiBare/',
                                                    'monthly_mean_fire_prob_', island[[i]], '_', names(model_list)[[M]], '_month', month[[m]], '_SE.tif'), overwrite = TRUE)
